...
int main(int argc, char* argv[])
{
    ...
    size = (high_value - low_value) / 2 + 1;/* 只需要考虑偶数即可，因此只需一半的空间 */
    ...
    /* 优化二：消除广播
	   核心思路：每个进程分别计算出3到sqrt(n)范围内的偶数，则可减少进程间广播开销 */
	...
	/* 优化三：cache优化
	   核心思想：每个进程先将cache大小相近的范围内的素数筛选完，这样可减少内存访问次数，提高运行效率
	 */
    ...
    for (k = 0; k < size; k += chunk) {
        ...
        for (j = 0; j < common_marked_size; j++) {
            ...
            */
            ...
            for (i = first + k; i < size && i < first + k + chunk; i += prime)marked[i] = 1;
        }
    }
